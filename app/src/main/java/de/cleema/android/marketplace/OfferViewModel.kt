...
fun Result<de.cleema.android.core.models.MarketItem>.toUiState(isRedeemed: Boolean): OfferUiState = fold(
    onSuccess = {
        OfferUiState.Success(
            if (isRedeemed) {
                it.copy(voucherRedemption = MarketItem.VoucherRedemption.Exhausted)
            } else {
                it
            }
        )
