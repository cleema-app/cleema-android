...
class DefaultQuizRepository @Inject constructor(
    ...
    override fun getQuizStream(): Flow<Result<de.cleema.android.core.models.QuizState>> = flow {
        emit(quizDataSource.getQuizState().map { it.toQuizState() })
        broadcaster.events.map {
            quizDataSource.getQuizState().map { it.toQuizState() }
        }.collect(this)
    }.flowOn(ioDispatcher)
